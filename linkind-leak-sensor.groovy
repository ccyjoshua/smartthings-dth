 ...
 *
 * Update history:
 * 12/03/2021 - V0.0   - Initial version
 * 
 *
 * Get updates from:
 * https://github.com/dfung/smartthings-dth/blob/main/linkind-leak-sensor.groovy
 *
 * Based on hubitat driver from:
...
def refresh() {
    ...
    return zigbee.readAttribute(zigbee.POWER_CONFIGURATION_CLUSTER, BATTERY_PERCENT_ATTR) +
...
def configure() {
    ...
    cmds += zigbee.configureReporting(0x0001, 0x0020, DataType.UINT8, 3600, 3600, 1)   // Configure Battery Voltage - Report once per Xhrs or if a change of 1% detected
    cmds += zigbee.configureReporting(0x0001, 0x0021, DataType.UINT8, 3600, 3600, 1)   // Configure Battery Percentage - Report once per Xhrs or if a change of 1% detected
    
    return cmds + refresh() + zigbee.batteryConfig()
}
